...
def calculate(Kd=0.001, etaINF1=0.001, B1=4.691, p=1.0, Kvisc=0.0014, EtaZero=0.0014, EtaInf=0.0014, B2=1.298701, n=1.0, wn=7e-05, Ln=6e-05, H=8e-05, wcont=7e-05, wdisp=6e-05, wout=0.00011, RhoO=1640, omega=0.006, sigmaEQ=0.052, Qw=6.116e-11, QoilStart=2.502e-11, QoilEnd=1.668e-10, QoilStep=2.78e-11, Ms=7.5, GAMMAinf=8e-06, Kads=600, Kdes=8.57e-05, Enth=5.629, m=0.06, CMC=0.128, is_ionic=True, debug=False, reporter=None):
    ...
    # (*Reyn[Qoil_]:=RhoO*(Qoil/H/wn)*wn/etaoNN[Qoil]*)
    def Reyn(Qoil):
        return RhoO*(Qoil/H/wn)*wn/etaoNN(Qoil)
    def xi(Qoil):
        z1 = 2.345 * n / (2.423 * n + 0.918)
        return 24/Reyn(Qoil)*(1+0.15*(Reyn(Qoil)) ** z1) # (*+Del(Qoil)+Drel*)

    def CAF(Qoil, Tdrop=Tdrop):
        return CaNC(Qoil, Tdrop) ** (2/3) / CaND(Qoil, Tdrop) ** (2/3) * (1 + 3.35 * CaND(Qoil, Tdrop) ** (1/3)) ** 2

    def VcontSq(Qoil, Tdrop=Tdrop):
        return (Qoil/H/HF(Qoil)/(wn-wjet0solF(Qoil))) ** 2 + (Qoil/H/HF(Qoil)/(wn-wjet0solF(Qoil))) ** 2 * (wn/wout) ** 2
